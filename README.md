...
const Calendar: React.FC = () => {
  ...
  const handleEventDragStop = (info: any) => {
    setIsDraggingOver(false);
    const target = info.jsEvent.target || 
                  (info.jsEvent.changedTouches?.[0]?.target);
    if (target && isInTrashBin(target)) {
      setEventToDelete(info.event);
      setShowDeleteConfirm(true);
    }
  };
  ...
  const checkFormComplete = (eventData: any) => {
    return (
      eventData.extendedProps.instructor &&
      eventData.extendedProps.duration &&
      eventData.extendedProps.location
    );
  };

  // Update ボタンをクリックしたときの処理
  const handleUpdateClick = () => {
    if (!selectedEventInfo) return;
    const isComplete = checkFormComplete(selectedEventInfo.plainObject);
    if (!isComplete) {
      toast.warning('未入力項目があります', {
        position: "top-center",
        autoClose: 3000,
...
文件: src/components/DraggableEvents.tsx
======================================
import React, { useEffect } from 'react';
import { Draggable } from '@fullcalendar/interaction';
import '../styles/draggable-events.css'; // CSS をインポート
...
return (
  <div id="external-events">
    <p><strong>イベントをカレンダーにドラッグ</strong></p>
    <div className="fc-event event-a">会議A</div>
    <div className="fc-event event-b">研修B</div>
    <div className="fc-event event-c">打ち合わせC</div>
    <div className="fc-event event-d">プレゼンテーションD</div>
    <div className="fc-event event-e">ワークショップE</div>
    <div className="fc-event event-f">セミナーF</div>
  </div>
);
}
...
文件: src/components/EventEditor.tsx
==================================
import React, { useState, useRef, useEffect } from 'react';
import '../styles/EventEditor.css';
...
const EventEdit: React.FC<EventEditProps> = ({ 
  ...
  const handleSave = () => {
    const updatedEvent = {
      ...
      extendedProps: {
        ...event.extendedProps,
        department: formData.department || event.extendedProps.department,
        instructor: formData.instructor || event.extendedProps.instructor,
        duration: formData.duration || event.extendedProps.duration,
        location: formData.location || event.extendedProps.location
      }
    };
    console.log("保存内容確認:", formData);
    onSave(updatedEvent);
  };
...
文件: src/components/TopNavBar.tsx
================================
...
文件: src/styles/draggable-events.css
===================================
#external-events {
  padding: 10px;
  width: 200px;
...
<html lang="en">
  <head>
    ...
    <link rel="icon" href="./src/resources/favicon.ico" type="image/x-icon">
    <title>スケジューラー</title>
  </head>
  <body>
    <div id="root"></div>
